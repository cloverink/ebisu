...
if ( ! function_exists( 'storefront_post_content' ) ) {
    ...
    echo "<div class='recipes-so'>";
      echo "<h6>SHARE THIS RECIPE</h6>";
      echo '<a href="" class="fb"></a>';
      echo '<a href="" class="tw"></a>';
    echo "</div>";
...
if ( ! function_exists( 'ebisu_slide_relate_product' ) ) {

  function ebisu_slide_relate_product( ) {
    ...
    if ( is_woocommerce_activated() ) {
      ...
      echo "<h4 class='header-prod-related'>Related products</h4>";

      echo "<div class='slider-container hide prod-relate'>";
        echo "<a href='#!' class='entypo-left-open-big'></a>";
        echo "<a href='#!' class='entypo-right-open-big'></a>";
        echo "<div>";

        ebisu_getproduct_slide_html($products, 'prod-relate');
...
if ( ! function_exists( 'ebisu_get_product_new_arrivals' ) ) {
  function ebisu_get_product_new_arrivals( ) {
    $args = array(
      'post_type' => 'product',
      'stock' => 1,
      'posts_per_page' => 10,
...
if ( ! function_exists( 'ebisu_get_product_best_seller' ) ) {
  function ebisu_get_product_best_seller( ) {
    $args = array('post_type' => 'product','posts_per_page' => 10, 'meta_key' => 'total_sales', 'orderby' => 'meta_value_num');
    return get_posts( $args ); 
  }
}

if ( ! function_exists( 'ebisu_slide_best_seller' ) ) {

  function ebisu_slide_best_seller( ) {

    if ( is_woocommerce_activated() ) {

      $products = ebisu_get_product_best_seller();

      echo "<div class='slider-container hide prod-best-seller'>";
        echo "<a href='#!' class='entypo-left-open-big'></a>";
        echo "<a href='#!' class='entypo-right-open-big'></a>";
        echo "<div>";

        ebisu_getproduct_slide_html($products, 'prod-best-seller');
...
if ( ! function_exists( 'ebisu_slide_our_product' ) ) {

  function ebisu_slide_our_product( ) {

    if ( is_woocommerce_activated() ) {


      echo "<div class='slider-container hide prod-out-product'>";
        echo "<a href='#!' class='entypo-left-open-big'></a>";
        echo "<a href='#!' class='entypo-right-open-big'></a>";
        echo "<div>";

        ebisu_getproduct_bycat('meat');
        ebisu_getproduct_bycat('seafood');
        ebisu_getproduct_bycat('processed-food');
        ebisu_getproduct_bycat('ingredients');
        ebisu_getproduct_bycat('seasoning');
        ebisu_getproduct_bycat('dessert-beverage');
...
if ( ! function_exists( 'ebisu_getproduct_bycat' ) ) {
  function ebisu_getproduct_bycat($cat_name, $posts_per_page = 10) {

    if ( is_woocommerce_activated() ) {

      $args     = array( 'post_type' => 'product', 'posts_per_page' => $posts_per_page, 'product_cat' => $cat_name );
      $products = get_posts( $args ); 

      ebisu_getproduct_slide_html($products, 'prod-' . $cat_name);
...
if ( ! function_exists( 'ebisu_getproduct_slide_html' ) ) {
  function ebisu_getproduct_slide_html($products, $class = "") {
    ...
    foreach ($products as $k => $v) {
      ...
      echo "<a href='$prod_url' class='btn-shopnow'>shop now</a>";
      echo "</div>";
    }
